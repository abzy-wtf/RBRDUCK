...
1. **Prep:** Within your Richard Burns install folder, create a new folder called 'Apps'.
2. **Download:** Download the latest release of RBRDUCK from the [releases page](https://github.com/abzy-wtf/RBRDUCK/releases) to the newly-created Apps folder.
3. **Extract:** Extract the contents of the downloaded release by right-clicking and choosing *'Extract to RBRDUCK\'*. You should now have *\Apps\RBRDUCK\\* within your RBR directory.
...
7. **Inject:** Copy the desired JanneMod V3 config files (`pacenote` and `ranges` folders) into *\Apps\RBRDUCK\template\Plugins\Pacenote\config\\*.
8. **Deploy:** Copy the generated framework into your RBR directory (`Audio` and `Plugins` folders).

##### Why RBRDUCK?

- With 963 individual calls, it is now possible to create a voicepack covering all 42 cornering systems in a single evening, variants included.
